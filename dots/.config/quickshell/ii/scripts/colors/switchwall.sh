...
THUMBNAIL_DIR="$RESTORE_SCRIPT_DIR/thumbnails"
    ...
    if [ -f "$SHELL_CONFIG_FILE" ]; then
        if [ -n "$monitor" ]; then
            ...
            jq --arg name "$monitor" --arg path "$path" '
                .background.thumbnailsByMonitor = (
                    (.background.thumbnailsByMonitor // []) | map(select(.monitor != $name)) + [{"monitor": $name, "path": $path}]
                )' "$SHELL_CONFIG_FILE" > "$SHELL_CONFIG_FILE.tmp" && mv "$SHELL_CONFIG_FILE.tmp" "$SHELL_CONFIG_FILE"
        else
            # Set global thumbnail path
...
switch() {
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if is_video "$imgpath"; then
            ...
            # QtMultimedia approach only needs ffmpeg for thumbnail extraction
            missing_deps=()
    ...
    # Generate global colors (SCSS)
    python3 "$SCRIPT_DIR/generate_colors_material.py" "${generate_colors_material_args[@]}" \
        > "$STATE_DIR"/user/generated/material_colors.scss
    
    # Generate per-monitor colors (JSON) if monitor is specified
    if [[ -n "$target_monitor" ]]; then
        python3 "$SCRIPT_DIR/generate_colors_material.py" "${generate_colors_material_args[@]}" --json \
            > "$STATE_DIR"/user/generated/colors_"${target_monitor}".json
    fi
